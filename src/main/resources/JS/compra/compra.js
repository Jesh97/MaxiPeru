    ...
    function actualizarTotales() {
        ...
        if (hayDescuento && tipoDescuento === 'global' && tipoValorDescuentoSelect && valorDescuentoInput) {
            ...
            if (tipoValor === 'porcentaje') {
                descuentoGlobalMonto = baseDescuentoGlobal * (valorDescuento / 100);
            } else {
                descuentoGlobalMonto = valorDescuento;
            }
        }

        descuentoGlobalMonto = Math.min(descuentoGlobalMonto, baseDescuentoGlobal);

        const subtotalConDescuento = subtotalSinIgvInicial - descuentoGlobalMonto;
        const totalCompraFinalCalculado = subtotalConDescuento + totalIgvCalculado;
        ...
        const resumenDescuentoGlobal = $('#resumenDescuentoGlobal');
        const filaDescuentoGlobal = $('#filaDescuentoGlobal');
        ...
        if (resumenDescuentoGlobal) {
            resumenDescuentoGlobal.textContent = formatCurrency(descuentoGlobalMonto, 'PEN');
        }

        if (filaDescuentoGlobal) {
            if (descuentoGlobalMonto > 0) {
                filaDescuentoGlobal.style.display = 'flex';
            } else {
                filaDescuentoGlobal.style.display = 'none';
            }
        }
        ...
        if (subtotalSinIgvEl) subtotalSinIgvEl.textContent = formatCurrency(subtotalConDescuento, 'PEN');
