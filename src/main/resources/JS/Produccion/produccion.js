    ...
    function calculateTotalConsumption(row) {
        ...
        const theoreticalQty = parseFloat(theoreticalCell.getAttribute('data-teorica')) || 0;
        const realQty = parseFloat(realInput.value) || 0;
        const unit = theoreticalCell.getAttribute('data-unit');
        const totalQty = theoreticalQty + realQty;
        const finalTotalDisplay = totalQty.toFixed(4) + ' ' + unit;
    ...
    function adjustQuantity(inputId, delta) {
        ...
        if (input.classList.contains('real-consumption')) {
        ...
        .then(data => {
            if (data.length > 0) {
                data.forEach(insumo => {
                    ...
                    const initialTotalConsumedDisplay = totalReq.toFixed(4) + ' ' + insumo.unidad_nombre;
                    const inputId = `real-cons-${activeOrdenCode}-${insumo.id_articulo}-${Date.now()}`;
                    const newRow = tableBody.insertRow();
                    newRow.innerHTML = `
                        <form action="" method="POST" onsubmit="handleInsumoConsumption(event)">
                            ...
                            <td class="quantity-control-consumo">
                                <button type="button" class="btn-qty-minus" onclick="adjustQuantity('${inputId}', -1)">-</button>
                                <input type="number" id="${inputId}" name="p_cantidad_adicional" class="real-consumption" value="0.0000" step="0.0001" oninput="calculateTotalConsumption(this.closest('tr'))" required>
                                <button type="button" class="btn-qty-plus" onclick="adjustQuantity('${inputId}', 1)">+</button>
                            </td>
                    ...
                    `;
                    newRow.querySelector('input[name="p_cantidad_adicional"]').dispatchEvent(new Event('input'));
                });
    ...
    function handleInsumoConsumption(event, rowOverride = null) {
        ...
        const totalConsumed = parseFloat(totalConsumptionDisplay.split(' ')[0]) || 0;
        ...
        .then(data => {
            ...
            submitButton.disabled = true;
            if (data.success) {
                formRow.style.backgroundColor = '#e6ffe6';
            } else {
                formRow.style.backgroundColor = '#ffe6e6';
    ...
    function calculateLooseContainers() {
        ...
        if (envasesSueltos > 0) {
            ...
            looseInputHidden.value = envasesSueltos;
    ...
    function submitFinalLotRegistration(event) {
        ...
        .then(data => {
            if (data.success) {
                alert("Registro final de Lote Guardado. La orden está\nlista para cerrarse.");
            } else {
    ...
    function loadRecetasList() {
        ...
        const params = new URLSearchParams({ action: 'listar_recetas' });
                ...
                finalRecetas.forEach(receta => {
                    ...
                    newRow.innerHTML = `
                        ...
                        <td>
                            ...
                            btn-compact btn-danger" onclick="confirmDeactivateReceta(${receta.id_receta}, '${nombreGenerico}')"><i class="fas fa-times"></i> Desactivar</button>
                        </td>
                    `;
                });
            } else {
            ...
            if (data.length > 0) {
                data.forEach(insumo => {
                    ...
                    newRow.innerHTML = `
                        ...
                        <td>
                            <button type="button" class="btn-submit btn-compact btn-info" onclick="openEditModal({ id_detalle: ${insumo.id_detalle_receta}, nombre_insumo: '${insumoNombreEscapado}', cantidad: ${insumo.cantidad_requerida}, unidad: '${insumo.unidad_nombre}' })"><i class="fas fa-pencil-alt"></i> Editar</button>
    ...
    function submitEditDetalleReceta(event) {
        ...
        .then(data => {
            if (data.success) {
                alert("Detalle de receta modificado exitosamente.");
                closeEditModal();
            } else {
                document.getElementById('edit-error-message').textContent = "Error: " + (data.message || "Detalle desconocido.");
            }
        })
        .catch(error => {
            document.getElementById('edit-error-message').textContent = "Error de comunicación: " + error.message;
        });
    }
    ...
    function loadOrdenesList() {
        alert("Función para cargar el listado de órdenes no implementada aún en el backend. (Paso 6)");
    }