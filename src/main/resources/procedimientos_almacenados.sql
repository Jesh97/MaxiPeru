...
CREATE PROCEDURE sp_crear_orden(
    IN p_id_receta INT, IN p_id_articulo_producido INT,
    IN p_cant_prod DECIMAL(12,2), IN p_cant_prod_final_real DECIMAL(12,2),
...
BEGIN
    UPDATE orden_produccion
    SET merma_total = p_merma_cantidad, envases_sueltos_cantidad = p_envases_sueltos, estado = 'Empaque Cerrado'
...
BEGIN
    ...
    IF v_abreviatura IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error: Artículo no válido o sin Producto Maestro asociado.';
    END IF;
    ...
    INSERT INTO control_remesa_lote (fecha_actual, ultimo_numero_remesa)
